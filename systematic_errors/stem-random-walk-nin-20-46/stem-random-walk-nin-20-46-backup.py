...
model_dir = "//flexo.ads.warwick.ac.uk/Shared41/Microscopy/Jeffrey-Ede/models/stem-random-walk-nin-20-46/"
...
def adjusted_mse(img1, img2):
    ...
    #Track average MSEs
    global adjusted_mse_counter;
    adjusted_mse_counter += 1
    avg_errors = tf.reshape(
        tf.constant(imread(r"Z:\Jeffrey-Ede\models\stem-random-walk-nin-20-43\errors.tif"), dtype=tf.float32), 
        (1,512,512,1))
    avg_errors = tf.image.resize_images(avg_errors, img1.get_shape()[1:-1], align_corners=True)
    scale = tf.reshape(avg_errors, img1.get_shape())**2
    ...
    mse = tf.reduce_mean( tf.stop_gradient(scale)*squared_errors )
            ...
            with tf.Session(config=sess_config) as sess:
                ...
                while True:
                    ...
                    while time.time()-time0 < modelSavePeriod:
                        ...
                        message = "NiN-46, Iter: {}, MSE_in: {}, MSE: {}, Wass G: {}, Wass D: {}, Val: {}".format(
