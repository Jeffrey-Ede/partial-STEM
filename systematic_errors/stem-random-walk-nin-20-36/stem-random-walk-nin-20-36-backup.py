...
model_dir = "//flexo.ads.warwick.ac.uk/Shared41/Microscopy/Jeffrey-Ede/models/stem-random-walk-nin-20-36/"
...
def experiment(feature, ground_truth, mask, learning_rate_ph, discr_lr_ph, beta1_ph, 
    ...
    wass_loss_for_gen_outer *= 5
    wass_loss_for_gen_inner *= 5

    mse_inner = 100*tf.losses.mean_squared_error(blur_truth_small, output_inner)
    mse_inner = 2.*tf.cond( mse_inner < 1, lambda: mse_inner, lambda: tf.sqrt(mse_inner+1.e-8) )
    #mse_inner = tf.minimum(mse_inner, 50)

    mse_outer = 100*tf.losses.mean_squared_error(blur_truth, output_outer)
    ...
    mse_outer_together = 100*tf.losses.mean_squared_error(blur_truth, blur_output_outer)
...
def gen_lq(img0):
    ...
    detection = mask * ( mask*img0 + 2*(1-mask)*np.random.rand(*img0.shape)*img )
    ...
    with open(log_file, 'a') as log:
        ...
        with tf.control_dependencies(update_ops):
            ...
            with tf.Session(config=sess_config) as sess:
                ...
                exp_dict = experiment(img_ph[0], img_truth_ph[0], img_mask_ph[0], learning_rate_ph, discr_learning_rate_ph,
                                      beta1_ph, discr_beta1_ph, norm_decay_ph, train_outer_ph, ramp_ph, initialize=True)
                ...
                #saver.restore(sess, tf.train.latest_checkpoint("Z:/Jeffrey-Ede/models/notable_ckpts/model/"))

                counter = 987_798#844_135#769_146#706_831#665787#608_254#564_544#507_119#697_088#516_376#314_088
                val_counter = 0
                ...
                while True:
                    ...
                    while time.time()-time0 < modelSavePeriod:
                        ...
                        message = "NiN-36, Iter: {}, MSE_in: {}, MSE: {}, Wass G: {}, Wass D: {}, Val: {}".format(
